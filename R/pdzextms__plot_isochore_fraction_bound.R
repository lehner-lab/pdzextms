...
#' @export
#' @import data.table
pdzextms__plot_isochore_fraction_bound <- function(
  mochi_outpath,
  input_dt,
  RT = 0.001987*(273+24),
  report_outpath,
  ...
  #Predicted fitness
  pred_fitness_list <- pdzextms__predict_fitness(
  ...
  wt_p_bound <- pdzextms__fraction_bound(input_dt[id=="-0-",f_dg_pred][1], input_dt[id=="-0-",b_dg_pred][1], RT = RT)
  single1_p_bound <- pdzextms__fraction_bound(input_dt[id=="-0-",f_dg_pred+1][1], input_dt[id=="-0-",b_dg_pred+1][1], RT = RT)
  single2_p_bound <- pdzextms__fraction_bound(input_dt[id=="-0-",f_dg_pred+1.5][1], input_dt[id=="-0-",b_dg_pred+2.5][1], RT = RT)
  double_p_bound <- pdzextms__fraction_bound(input_dt[id=="-0-",f_dg_pred+2.5][1], input_dt[id=="-0-",b_dg_pred+3.5][1], RT = RT)
