...
#' @export
#' @import data.table
pdzextms__predict_fitness <- function(
  mochi_outpath,
  folding_energy,
  binding_energy,
  RT = 0.001987*(273+24)
  ...
  if(!is.null(folding_energy)){
    pred_list[["fraction_folded"]] <- pdzextms__fraction_folded(folding_energy, RT)
  ...
  if(!is.null(folding_energy) & !is.null(binding_energy) & length(folding_energy) == length(binding_energy)){
    pred_list[["fraction_bound"]] <- pdzextms__fraction_bound(folding_energy, binding_energy, RT)
