...
#' @export
#' @import data.table
pdzextms_thermo_model_results <- function(
  mochi_outpath,
  temperature = 30,
  ...
  #Display status
  message(paste("\n\n*******", paste("running stage: pdzextms_thermo_model_results for", domain_name), "*******\n\n"))

  #Create output directory
  pdzextms__create_dir(pdzextms_dir = outpath)
  ...
  pdzextms__plot_mutation_distributions(
    input_dt = fitness_dt,
    report_outpath = outpath,
    colour_scheme = colour_scheme)

  #Load model results
  pred_dt <- pdzextms__get_model_results(
    input_folder = mochi_outpath, 
    input_dt = fitness_dt, 
    RT = RT)

  #Call confident ddGs
  pred_dt_conf <- pdzextms__define_confident_free_energies(
    input_dt = pred_dt, 
    report_outpath = outpath, 
  ...
  pdzextms__plot_additive_trait_binding(
    mochi_outpath = mochi_outpath,
    input_dt = pred_dt_conf, 
    RT = RT,
    report_outpath = outpath,
  ...
  pdzextms__plot_isochore_fraction_bound(
    mochi_outpath = mochi_outpath,
    input_dt = pred_dt_conf, 
    RT = RT,
    report_outpath = outpath,
  ...
  pdzextms__plot_validation_scatter(
    input_dt = pred_dt_conf, 
  ...
  pred_dt_conf[, id_ref := pdzextms__get_reference_id(pred_dt_conf[,.(id, mut_order)], position_offset)]
  ...
  pred_dt_conf[mut_order==1 & !duplicated(id), b_ddg_pred_sig := p.adjust(pdzextms__pvalue(b_ddg_pred, b_ddg_pred_sd), method = "BH")<0.05]
  pred_dt_conf[mut_order==1 & !duplicated(id), f_ddg_pred_sig := p.adjust(pdzextms__pvalue(f_ddg_pred, f_ddg_pred_sd), method = "BH")<0.05]
